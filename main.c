...
int main() {
    ...
    if (load_config(&master.config) < 0) {
        LOG("Failed to load_config.\n");
        return -1;
    }

    // 加载协议 // master_init
    if (load_plugins(&master) < 0) {
        LOG("Failed to load_plugins.\n");
        return -1;
    }
    ...
    if (load_task_callmodel(&master.callmodel) < 0) {
        LOG("Failed to load_task_callmodel.\n");
        return -1;
    }

    // 1. 读取任务设置并交给plugin处理
    if (load_task_setting(&master.settings) < 0) {
        LOG("Failed to load_task_setting.\n");
        return -1;
    }

    // 2. 读取任务变量
    if (load_task_vars(&master.vars) < 0) {
        LOG("Failed to load_task_vars.\n");
        return -1;
    }

    // 3. 读取任务脚本流程
    if (load_task_script(&master.script) < 0) {
        LOG("Failed to load_task_script.\n");
        return -1;
    }
    ...
    if (plugins_script_init(&master.settings, &master.script, master.plugins, master.num_plugins) < 0) {
        LOG("Failed to plugins_script_init.\n");
        return -1;
    }

    // 插件加载任务设置 // task_init()
    if (plugins_task_init(master.plugins, master.num_plugins) < 0) {
        LOG("Failed to plugins_task_init.\n");
        return -1;
    }
    ...
    if (do_task_callmodel(&master.callmodel) < 0) {
        LOG("Failed to do_task_callmodel.\n");
        return -1;
    }
